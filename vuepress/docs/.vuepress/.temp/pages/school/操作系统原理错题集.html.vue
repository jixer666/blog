<template><div><h1 id="操作系统原理错题集" tabindex="-1"><a class="header-anchor" href="#操作系统原理错题集"><span>操作系统原理错题集</span></a></h1>
...
<p>**整体式：**优点【1、模块设计、编码和调试独立；2、模块之间可以自由调用】，缺点【信息传递随意，维护和更新困难】</p>
<p>**层次式：**优点【1、结构清晰，避免循环调用；2、整体问题局部化，系统的正确性容易保证；3、有利于操作系统的维护。扩充、移植】，缺点【效率低】</p>
<p>**微内核结构：**优点【便于扩充，移植，安全性高，便于维护，系统更加稳定】，缺点【效率低】</p>
</blockquote>
<h2 id="第二单元-进程" tabindex="-1"><a class="header-anchor" href="#第二单元-进程"><span>第二单元 进程</span></a></h2>
<p>【单选题】一个进程被唤醒意味着( )（13.0分）</p>
<ul>
...
<p>正确答案： A 我的答案：A</p>
<p><img src="https://gitee.com/lijunxi666/picture-bed/raw/master/school/image-20200712142536609.png" alt=""></p>
<hr>
...
<blockquote>
<p>内核级线程是<a href="https://wenwen.sogou.com/s/?w=%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F%E5%86%85%E6%A0%B8&amp;ch=ww.xqy.chain" target="_blank" rel="noopener noreferrer">操作系统内核</a>实现、管理和调度的一种线程。由于有操作系统管理，所以操作系统是知道线程的存在，并为其安排<a href="https://wenwen.sogou.com/s/?w=%E6%97%B6%E9%97%B4%E7%89%87&amp;ch=ww.xqy.chain" target="_blank" rel="noopener noreferrer">时间片</a>，管理与其有关的<a href="https://wenwen.sogou.com/s/?w=%E5%86%85%E6%A0%B8%E5%AF%B9%E8%B1%A1&amp;ch=ww.xqy.chain" target="_blank" rel="noopener noreferrer">内核对象</a>。因为内核级线程是由内核来管理，所以每次线程创建、切换都要执行一个模式切换<a href="https://wenwen.sogou.com/s/?w=%E4%BE%8B%E7%A8%8B&amp;ch=ww.xqy.chain" target="_blank" rel="noopener noreferrer">例程</a>，所以内核级线程效率比较低，而且内核级线程的调度是由操作系统的设计者来决定的，所以缺乏灵活性。但是内核级线程有一个有点就是当一个进程的某个线程因为一个<a href="https://wenwen.sogou.com/s/?w=%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8&amp;ch=ww.xqy.chain" target="_blank" rel="noopener noreferrer">系统调用</a>或者<a href="https://wenwen.sogou.com/s/?w=%E7%BC%BA%E9%A1%B5%E4%B8%AD%E6%96%AD&amp;ch=ww.xqy.chain" target="_blank" rel="noopener noreferrer">缺页中断</a>而阻塞时，不会导致该进程的所有<a href="https://wenwen.sogou.com/s/?w=%E7%BA%BF%E7%A8%8B%E9%98%BB%E5%A1%9E&amp;ch=ww.xqy.chain" target="_blank" rel="noopener noreferrer">线程阻塞</a>。 内核级线程的优点： 较好的并行能力，一个进程内的线程阻塞不会影响该进程内的其他线程 内核级线程的缺点： 线程管理的开销过大，缺乏灵活性。 用户级线程是通过运行在<a href="https://wenwen.sogou.com/s/?w=%E7%94%A8%E6%88%B7%E6%80%81&amp;ch=ww.xqy.chain" target="_blank" rel="noopener noreferrer">用户态</a>的运行时库来管理的，其优点是，线程的一切（包括调度、创建）都可以完全由用户自己决定，所以具有较高的灵活性。而且由于是在用户态上进行管理，所以就省去了内核管理的开销，所以具有高效率。 但是用户级线程有一个致命的缺点：一个进程内的某一个线程阻塞将导致整个进程内的所有线程全部阻塞。而且由于用户级线程没有时间片概念，所以每个线程必须运行一段时间后将CPU让个其他的线程使用，否则，该线程将独占CPU。 用户级线程的优点： 有较高的灵活性和高效率 用户级线程的缺点： 较差并发能力</p>
</blockquote>
<hr>
<p>【判断题】同样的电脑上,用两个线程和一个线程进行同样的数学运算,两个线程的算的更快。正确答案：<em>×</em> 我的答案：<em>×</em></p>
<h2 id="第三单元-进程的并发" tabindex="-1"><a class="header-anchor" href="#第三单元-进程的并发"><span>第三单元 进程的并发</span></a></h2>
<p>【单选题】<br>
关于临界区说法正确的是（）</p>
<ul>
...
<hr>
<p>【单选题】<br>
若信号量 S 的初值为 3,当前值为−2,则表示有（ ）等待进程</p>
<ul>
<li>_A、_1</li>
<li>_B、_2</li>
<li>_C、_3</li>
...
<hr>
<p>【单选题】<br>
设与某资源相关联的信号量初值为3，当前值为1，若M表示该资源的可用个数，N表示等待资源的进程数，则M，N分别是( )</p>
<ul>
<li>_A、_1，0</li>
<li>_B、_3,1</li>
<li>_C、_1,2</li>
...
<hr>
<p>【单选题】<br>
如果4个进程共享一个程序段，每次最多允许2个进程进入，用PV操作做同步，那么信号量的取值范围是（ ）</p>
<ul>
<li>_A、_2,1,0,-1</li>
<li>_B、_3,2,1,0,-1</li>
...
<hr>
<p>【单选题】<br>
A初始值为3，signal操作时，加1后 若信号量A值为（    ）， 则要去唤醒某个进程</p>
<ul>
<li>_A、_0</li>
<li>_B、_1</li>
<li>_C、_2</li>
...
<hr>
<p>【判断题】<br>
之所以提出硬件方式解决方案的一个原因是因为用纯软件方法解决同步很困难。我的答案：√</p>
<h2 id="第四单元-进程通信和调度" tabindex="-1"><a class="header-anchor" href="#第四单元-进程通信和调度"><span>第四单元 进程通信和调度</span></a></h2>
<p>【单选题】需要用户自己做同步互斥的IPC方式是（ ）</p>
<ul>
<li>_A、_共享存储器</li>
<li>_B、_管道文件</li>
<li>_C、_消息传递</li>
...
<p>&gt; 响应比高者优先调度算法（RRHF）：<code v-pre>响应比=响应时间/运行时间=1+等待时间/运行时间</code> ，相应比越高越容易调度。优点：有利于短作业、有利于等待已久的作业、有利于长作业</p>
<p>进程调度算法：</p>
<p>&gt; 优先数高者优先调度算法（HPF）</p>
...
<p><code v-pre>t = tc(作业完成时间) - ts(作业提交时间)</code></p>
<p><code v-pre>t = tw(作业等待时间) + tr(作业运行时间)</code></p>
<p>平均周转时间：<code v-pre>t' = (t1 + t2 + t3 + ...) / n</code></p>
<p>带权周转时间：<code v-pre>w = t(周转时间) / tr(程序运行时间)</code></p>
<p>平均带权周转时间：<code v-pre>w' = (w1 + w2 + w3 + ...) / n</code></p>
</blockquote>
<h2 id="第五单元-死锁" tabindex="-1"><a class="header-anchor" href="#第五单元-死锁"><span>第五单元 死锁</span></a></h2>
<p>【单选题】以下（ ）方法属于预防死锁</p>
<ul>
<li>_A、_银行家算法</li>
<li>_B、_有序分配</li>
...
<h2 id="第六单元-存储器管理" tabindex="-1"><a class="header-anchor" href="#第六单元-存储器管理"><span>第六单元 存储器管理</span></a></h2>
<p>【单选题】以下说法正确的是（）</p>
<ul>
...
<hr>
<p>【填空题】已知内存分区如下图</p>
<p><img src="https://gitee.com/lijunxi666/picture-bed/raw/master/school/e5a75bd81a0c294cde55fea58f51c6f6.png" alt=""></p>
...
<p>正确答案： A 我的答案：A</p>
<blockquote>
<p>高速缓存(Cache)有L1 L2 L3共三级缓存,它的速度是和CPU同步的,而<a href="https://zhidao.baidu.com/search?word=%BC%C4%B4%E6%C6%F7&amp;fr=iknow_pc_qb_highlight" target="_blank" rel="noopener noreferrer">寄存器</a>本身是属于CPU的,比<a href="https://zhidao.baidu.com/search?word=%D6%F7%B4%E6&amp;fr=iknow_pc_qb_highlight" target="_blank" rel="noopener noreferrer">主存</a>快多了</p>
</blockquote>
<hr>
...
<hr>
<p>【判断题】高速缓存是软件的概念，是内存划分出来的一部分<br>
正确答案：<em>×</em> 我的答案：<em>×</em></p>
<blockquote>
...
<h2 id="第一章课后作业" tabindex="-1"><a class="header-anchor" href="#第一章课后作业"><span>第一章课后作业</span></a></h2>
<p>(单选题)操作系统最基本的特性是(    )</p>
<ul>
<li>A. 并发性</li>
<li>B. 虚拟性</li>
...
<h2 id="第二章课后作业" tabindex="-1"><a class="header-anchor" href="#第二章课后作业"><span>第二章课后作业</span></a></h2>
<p>(单选题, 4分)进程和程序说法错误的是</p>
<ul>
...
<h2 id="虚拟存储器课后作业" tabindex="-1"><a class="header-anchor" href="#虚拟存储器课后作业"><span>虚拟存储器课后作业</span></a></h2>
<p>分页虚拟存储管理系统中，页面的大小和可能产生的缺页中断次数（  ）</p>
<ul>
<li>A. 成正比</li>
<li>B. 成反比</li>
<li>C. 固定比</li>
...
<p><img src="https://gitee.com/lijunxi666/picture-bed/raw/master/school/2c28ddc532dac5a09f0a42c2233541db-2-1024x300.png" alt=""></p>
<p><img src="https://gitee.com/lijunxi666/picture-bed/raw/master/school/e18de3f0045ac2dfaaa487f341180a01-1-1024x298.png" alt=""></p>
<p><img src="https://gitee.com/lijunxi666/picture-bed/raw/master/school/701e681f811c4c8b91b2779b5582cbfb-1024x269.png" alt=""></p>
<hr>
...
<p><em>我的答案:</em> 错_正确答案:_ 错</p>
<h2 id="io设备课后作业" tabindex="-1"><a class="header-anchor" href="#io设备课后作业"><span>IO设备课后作业</span></a></h2>
<p>下面的选项中哪个不是设备驱动程序功能(  )</p>
<ul>
...
<p><img src="https://gitee.com/lijunxi666/picture-bed/raw/master/school/204609f6ffdce3ca8d3c46b38abf35ca-1.png" alt=""></p>
<ul>
<li>A. 200</li>
<li>B. 295</li>
<li>C. 300</li>
...
<p><em>正确答案:</em> CF</p>
<h2 id="其他题" tabindex="-1"><a class="header-anchor" href="#其他题"><span>其他题</span></a></h2>
<p>设从磁盘将一块数据传送到缓冲区所用实际为80μs，将缓冲区中数据传送到用户区所用时间为40μs，CPU处理数据所用的时间为3μs。则处理该数据，采用单缓冲传送磁盘数据，系统所用平均总时间（一共传输多块）为（120）。多缓冲为（80）</p>
<blockquote>
...
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">单缓冲区总时间=(磁盘写入缓冲区时间+缓冲区读出时间)*磁盘块数+CPU处理最后一块数据的时间=(80+60)*10+40=1440us</span>
<span class="line"></span>
<span class="line">双缓冲总时间=磁盘写入缓冲区时间*磁盘块数+读出最后一块数据时间+CPU分析最后一块数据时间=80*10+60+40=900us</span>
...
<p>操作系统--分别用FCFS，SSTF，SCAN，CSCAN算法计算总寻道长度和平均寻道长度例题（详细）：<a href="https://blog.csdn.net/weixin_45678463/article/details/111866092" target="_blank" rel="noopener noreferrer">https://blog.csdn.net/weixin_45678463/article/details/111866092</a></p>
<hr>
...
<hr>
<p>【判断题】索引文件对记录关键字和主文件记录都进行了排序,所以检索速度比顺序文件更快<br>
正确答案：<em>×</em> 我的答案：<em>√</em></p>
<blockquote>
<p>没有对主文件记录排序</p>
</blockquote>
<hr>
<p>【判断题】流式文件中的数据格式(如音乐文件)在不同的操作系统中(比如UNIX和Windows)是不一样的<br>
正确答案：<em>×</em> 我的答案：<em>×</em></p>
<blockquote>
...
<p>正确答案： A</p>
<hr>
<p>【判断题】FCB的有序集合 被称为文件目录<br>
正确答案：<em>√</em> 我的答案：<em>√</em></p>
<hr>
<p>【判断题】文件的显式链接存放方式比隐式链接方式更可靠<br>
正确答案：<em>×</em> 我的答案：<em>×</em></p>
<blockquote>
<p>都存在断链无法访问的情况, 隐式链接的链条存放在文件本身， 显示链接的链条存放在FAT表，一个是FAT表块坏了断链，一个是文件磁盘块坏了断链</p>
</blockquote>
<hr>
<p>【判断题】读取文件时,显式链接比隐式链接更快<br>
正确答案：<em>√</em> 我的答案：<em>√</em></p>
<blockquote>
<p>显式链接的FAT表可以存放到内存中</p>
</blockquote>
<h2 id="外部空间管理" tabindex="-1"><a class="header-anchor" href="#外部空间管理"><span>外部空间管理</span></a></h2>
<p>【单选题】有关顺序文件的说法正确的是(  )</p>
<ul>
...
<p>正确答案： D</p>
<hr>
<p>【判断题】磁盘文件连续分配会存在外零头, 采用紧凑, 可以提高文件处理效率<br>
正确答案：<em>×</em> 我的答案：<em>×</em></p>
<blockquote>
